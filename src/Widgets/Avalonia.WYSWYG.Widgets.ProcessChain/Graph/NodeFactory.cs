...
using NodeTemplateViewModel = Avalonia.WYSWYG.Widgets.ProcessChain.Graph.Editor.MVVM.NodeTemplateViewModel;
...
public partial class NodeFactory : INodeFactory
{
    ...
    */
        ...
        (double x, double y) position, double width = 60, double height = 60, Control nodeView = null)
    {
        ...
        var viewModel = NodeFactory.CreateViewModel(node, position, (width, height));

        NodeFactory.AddPins(topPins, viewModel, i => (NodeFactory.CalculateSinglePin(width, topPins.Length, i), 0));
        NodeFactory.AddPins(bottomPins, viewModel, i => (NodeFactory.CalculateSinglePin(width, bottomPins.Length, i), height));

        NodeFactory.AddPins(leftPins, viewModel, i => (0, NodeFactory.CalculateSinglePin(height, leftPins.Length, i)));
        NodeFactory.AddPins(rightPins, viewModel, i => (width, NodeFactory.CalculateSinglePin(height, rightPins.Length, i)));
