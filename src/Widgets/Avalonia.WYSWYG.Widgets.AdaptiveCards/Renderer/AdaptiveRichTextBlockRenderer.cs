...
public static class AdaptiveRichTextBlockRenderer
{
    public static Control Render(AdaptiveRichTextBlock richTB, AdaptiveRenderContext context)
    {
        ...
        {
            var textRun = inlineElement as AdaptiveTextRun;
    ...
    private static void AddInlineTextRun(TextBlock uiRichTB, AdaptiveTextRun textRun, AdaptiveRenderContext context)
    {
        ...
        var text = RendererUtilities.ApplyTextFunctions(textRun.Text, context.Lang);

        textRunSpan.Inlines.Add(text);

        // textRunSpan.Style = context.GetStyle($"Adaptive.{textRun.Type}");

        textRunSpan.FontFamily =
            new(RendererUtil.GetFontFamilyFromList(context.Config.GetFontFamily(textRun.FontType)));
        ...
        if (textRun.Italic) textRunSpan.FontStyle = FontStyle.Italic;

        if (textRunSpan.TextDecorations == null) textRunSpan.TextDecorations = new();

        if (textRun.Strikethrough) textRunSpan.TextDecorations.AddRange(TextDecorations.Strikethrough);

        if (textRun.Underline) textRunSpan.TextDecorations.AddRange(TextDecorations.Underline);

        if (textRun.Highlight) textRunSpan.SetHighlightColor(textRun.Color, textRun.IsSubtle, context);
    ...
    private static TextBlock CreateControl(AdaptiveRichTextBlock richTB, AdaptiveRenderContext context)
    {
        ...
        if (richTB.HorizontalAlignment != AdaptiveHorizontalAlignment.Left)
        {
            ...
            if (Enum.TryParse(richTB.HorizontalAlignment.ToString(), out alignment))
