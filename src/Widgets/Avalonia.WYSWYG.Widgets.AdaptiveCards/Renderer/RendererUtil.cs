...
internal static class RendererUtil
{
    ...
    {
        var fontFamilies = fontList.Split(',');
    ...
    public static Control ApplySelectAction(Control uiElement, AdaptiveElement element, AdaptiveRenderContext context)
    {
        ...
        else if (element is AdaptiveImage) selectAction = (element as AdaptiveImage).SelectAction;

        if (selectAction != null) return context.RenderSelectAction(selectAction, uiElement);
