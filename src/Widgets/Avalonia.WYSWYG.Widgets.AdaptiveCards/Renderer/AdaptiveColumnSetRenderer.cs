...
public static class AdaptiveColumnSetRenderer
{
    public static Control Render(AdaptiveColumnSet columnSet, AdaptiveRenderContext context)
    {
        ...
        var inheritsStyleFromParent = !columnSet.Style.HasValue;
        var hasPadding = false;
        if (!inheritsStyleFromParent)
        {
            hasPadding =
                AdaptiveContainerRenderer.ApplyPadding(border, uiColumnSet, columnSet, parentRenderArgs, context);
        ...
        for (var i = 0; i < columnSet.Columns.Count; ++i)
        {
            ...
            var uiContainer = context.Render(column);
            ...
            if (uiContainer != null)
            {
                ...
                if (uiColumnSet.ColumnDefinitions.Count > 0 &&
                    (column.Separator || column.Spacing != AdaptiveSpacing.None))
                {
                    var uiSep = new Grid();
                    ...
                    var spacing = context.Config.GetSpacing(column.Spacing);
                    uiSep.Margin = new(spacing / 2.0, 0, spacing / 2.0, 0);
                    ...
                    tag = new(uiSep, uiColumnSet);

                    uiColumnSet.ColumnDefinitions.Add(new() {Width = GridLength.Auto});
                    Grid.SetColumn(uiSep, uiColumnSet.ColumnDefinitions.Count - 1);
                    uiColumnSet.Children.Add(uiSep);
                }
                else
                {
                    tag = new(null, uiColumnSet);
                }
...
#pragma warning restore CS0618 // Type or member is obsolete
                ...
                {
                    columnDefinition = new() {Width = new(1, GridUnitType.Star)};
                }
                else if (width == AdaptiveColumnWidth.Auto.ToLower())
                {
                    columnDefinition = new() {Width = GridLength.Auto};
                }
                else
                {
                    if (double.TryParse(width, out var val) && val >= 0)
                        // Weighted proportion (number only)
                        columnDefinition = new() {Width = new(val, GridUnitType.Star)};
                    else if (width.EndsWith("px") &&
                             double.TryParse(width.Substring(0, width.Length - 2), out var pxVal) && pxVal >= 0)
                        // Exact pixel (number followed by "px")
                        columnDefinition = new() {Width = new((int) pxVal, GridUnitType.Pixel)};
                    else
                        columnDefinition = new() {Width = GridLength.Auto};
                }
                ...
                if (!string.IsNullOrEmpty(column.Id)) context.RenderedElementsWithId[column.Id] = uiContainer;
