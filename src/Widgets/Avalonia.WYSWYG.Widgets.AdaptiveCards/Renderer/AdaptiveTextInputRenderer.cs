...
public static class AdaptiveTextInputRenderer
{
    public static Control Render(AdaptiveTextInput input, AdaptiveRenderContext context)
    {
        ...
        if (input.IsMultiline)
        {
            ...
            textBox.SetValue(ScrollViewer.HorizontalScrollBarVisibilityProperty,
                ScrollBarVisibility.Disabled); //textBox.HorizontalScrollBarVisibility = ScrollBarVisibility.Disabled;
        }

        if (input.MaxLength > 0) textBox.MaxLength = input.MaxLength;
        ...
        if (input.InlineAction != null)
        {
            ...
            {
                context.Warnings.Add(new(-1, "Inline ShowCard not supported for InlineAction"));
            }
            else
            {
                if (context.Config.SupportsInteractivity &&
                    context.ActionHandlers.IsSupported(input.InlineAction.GetType()))
    ...
    public static Control RenderInlineAction(AdaptiveTextInput input, AdaptiveRenderContext context, Control textBox)
    {
        ...
        parentView.ColumnDefinitions.Add(new() {Width = new(1, GridUnitType.Star)});
        Grid.SetColumn(textBox, 0);
        parentView.Children.Add(textBox);

        // grid config for spacing
        var spacing = context.Config.GetSpacing(AdaptiveSpacing.Default);
        var uiSep = new Grid
        {
        ...
        parentView.ColumnDefinitions.Add(new() {Width = new(spacing, GridUnitType.Pixel)});
        ...
        if (input.InlineAction.IconUrl != null)
        {
            ...
            // adjust height
            textBox.Loaded += (sender, e) => { uiIcon.Height = textBox.Bounds.Height; };
        ...
        {
            context.InvokeAction(uiButton, new(input.InlineAction));

            // Prevent nested events from triggering
            e.Handled = true;
        };

        parentView.ColumnDefinitions.Add(new() {Width = GridLength.Auto});
...
{
    public AdaptiveTextInputValue(AdaptiveTextInput inputElement, Control renderedElement) : base(inputElement,
        renderedElement)
    {
    }
    ...
    public override bool Validate()
    {
        ...
        var textInput = InputElement as AdaptiveTextInput;
        ...
        if (textInput.MaxLength != 0) isValid = isValid && GetValue().Length <= textInput.MaxLength;
