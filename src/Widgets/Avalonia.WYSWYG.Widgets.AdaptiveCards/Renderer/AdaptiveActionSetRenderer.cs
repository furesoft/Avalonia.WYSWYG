...
public static class AdaptiveActionSetRenderer
{
    ...
    public static void AddRenderedActions(Grid uiContainer, IList<AdaptiveAction> actions,
        AdaptiveRenderContext context, AdaptiveInternalID actionSetId)
    {
        if (!context.Config.SupportsInteractivity)
            return;
        ...
        uiActionBar.HorizontalAlignment =
            (HorizontalAlignment) Enum.Parse(typeof(HorizontalAlignment), actionsConfig.ActionAlignment.ToString());
        ...
        uiActionBar.Margin = new(0, topMargin, 0, 0);
        ...
        var isInline = actionsConfig.ShowCard.ActionMode == ShowCardActionMode.Inline;

        var iPos = 0;

        if (primaryActions.Any())
            iPos = RenderPrimaryActions(uiContainer, context, actionSetId, actionsConfig, primaryActions, uiActionBar,
                isInline, iPos);

        if (secondaryActions.Any())
            iPos = RenderOverflowActions(uiContainer, context, actionSetId, actionsConfig, secondaryActions,
                uiActionBar, isInline, iPos);
    }

    private static int RenderPrimaryActions(Grid uiContainer, AdaptiveRenderContext context,
        AdaptiveInternalID actionSetId, ActionsConfig actionsConfig, List<AdaptiveAction> actions,
        UniformGrid uiActionBar, bool isInline, int iPos)
    {
        ...
        if (!allActionsHaveIcons) actionsConfig.IconPlacement = IconPlacement.LeftOfTitle;
        ...
        foreach (var action in actions)
        {
            ...
            if (rendereableAction != null)
            {
                if (rendereableAction is AdaptiveSubmitAction ||
                    ...
                    context.SubmitActionCardId[rendereableAction as AdaptiveAction] =
                        context.RenderArgs.ContainerCardId;
                ...
                if (uiAction == null)
                {
                    context.Warnings.Add(new(-1, $"action failed to render" +
                ...
                if (actionsConfig.ActionsOrientation == ActionsOrientation.Horizontal)
                {
                    if (uiActionBar.Children.Count > 0) // don't apply left margin to the first item
                        uiAction.Margin = new(actionsConfig.ButtonSpacing, 0, 0, 0);
                }
                else
                {
                    uiAction.Margin = new(0, actionsConfig.ButtonSpacing, 0, 0);
                }
                ...
                if (action is AdaptiveShowCardAction showCardAction && isInline)
                    CreateShowCardAction(uiContainer, context, actionSetId, actionsConfig, hasSeenInlineShowCard,
                        uiAction, showCardAction);
            }
        }
    ...
    private static int RenderOverflowActions(Grid uiContainer, AdaptiveRenderContext context,
        AdaptiveInternalID actionSetId, ActionsConfig actionsConfig, List<AdaptiveAction> actions,
        UniformGrid uiActionBar, bool isInline, int iPos)
    {
        ...
        foreach (var action in actions)
        {
            ...
            if (rendereableAction != null)
            {
                ...
                // vertical
                uiAction.BorderThickness = new(0);
                ...
                {
                    if (actionSetId != null)
                        hasSeenInlineShowCard = CreateShowCardAction(uiContainer, context, actionSetId, actionsConfig,
                            hasSeenInlineShowCard, uiAction, showCardAction);
                    else
                        context.Warnings.Add(new(-1, $"button's corresponding showCard" +
        ...
        {
            if (uiActionBar.Children.Count > 0) // don't apply left margin to the first item
                uiOverflow.Margin = new(actionsConfig.ButtonSpacing, 0, 0, 0);
        }
        else
        {
            uiOverflow.Margin = new(0, actionsConfig.ButtonSpacing, 0, 0);
        }
    ...
    private static bool CreateShowCardAction(Grid uiContainer, AdaptiveRenderContext context,
        AdaptiveInternalID actionSetId, ActionsConfig actionsConfig, bool hasSeenInlineShowCard, Button uiAction,
        AdaptiveShowCardAction showCardAction)
    {
        ...
        if (!hasSeenInlineShowCard)
        {
            ...
            context.PeerShowCardsInActionSet[actionSetId] = new();
        }

        hasSeenInlineShowCard = true;

        var uiShowCardContainer = new Grid();
        ...
        uiShowCardContainer.Margin = new(-padding, actionsConfig.ShowCard.InlineTopMargin, -padding, -padding);
        ...
        var currentParentCardId = context.RenderArgs.ContainerCardId;
    ...
    private static List<AdaptiveAction> GetActionsToProcess(IList<AdaptiveAction> actions, int maxActions,
        AdaptiveRenderContext context, AdaptiveActionMode mode)
    {
        // If the number of actions is bigger than maxActions, then log warning for it
        if (actions.Count > maxActions)
            context.Warnings.Add(new((int) AdaptiveWarning.WarningStatusCode.MaxActionsExceeded,
