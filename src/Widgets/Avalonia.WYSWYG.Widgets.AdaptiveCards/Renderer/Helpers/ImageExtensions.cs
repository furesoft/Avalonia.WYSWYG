...
namespace Avalonia.WYSWYG.Widgets.AdaptiveCards.Renderer.Helpers;
...
public static class ImageExtensions
{
    ...
    public static MemoryStream RenderToImage(this Control element, int width)
    {
        ...
        //// Question: Should we set the image metadata?
    ...
    public static async void SetBackgroundSource(this Grid grid, AdaptiveBackgroundImage adaptiveBackgroundImage,
        AdaptiveRenderContext context)
    {
        // Try to resolve the image URI
        var finalUri = context.Config.ResolveFinalAbsoluteUri(adaptiveBackgroundImage?.Url);
        ...
        if (finalUri.Scheme == "data")
        {
            ...
            var decodedDataUri = Convert.FromBase64String(encodedData);
            bitmap = new(new MemoryStream(decodedDataUri));
        }
        else
        {
        ...
        if (bitmap != null)
            switch (adaptiveBackgroundImage.FillMode)
            {
                case AdaptiveImageFillMode.RepeatVertically:
                    grid.GetObservable(Visual.BoundsProperty).Subscribe(bounds =>
                    {
                        if (grid.Background is ImageBrush brush)
                        {
                            if (bounds.Width > 0 && brush.DestinationRect.Rect.Width != bounds.Width)
                                brush.DestinationRect = new(0, 0, bounds.Width, bitmap.Size.Height,
                        ...
                        else
                        {
                            grid.Background = new ImageBrush(bitmap)
                            {
                                ...
                                DestinationRect = new(0, 0, bounds.Width, bitmap.Size.Height, RelativeUnit.Absolute)
                            };
                        }
                    });
                    break;
                    ...
                    grid.GetObservable(Visual.BoundsProperty).Subscribe(bounds =>
                    {
                        if (grid.Background is ImageBrush brush)
                        {
                            if (bounds.Height > 0 && brush.DestinationRect.Rect.Height != bounds.Height)
                                brush.DestinationRect = new(0, 0, bitmap.Size.Width, bounds.Height,
                                    RelativeUnit.Absolute);
                        }
                        else
                        {
                            grid.Background = new ImageBrush(bitmap)
                            {
                                ...
                                DestinationRect = new(0, 0, bitmap.Size.Width, bounds.Height, RelativeUnit.Absolute)
                            };
                        }
                    });
                    break;
                case AdaptiveImageFillMode.Repeat:
                    grid.Background = new ImageBrush(bitmap)
                    {
                        ...
                        DestinationRect = new(0, 0, bitmap.Size.Width, bitmap.Size.Height, RelativeUnit.Absolute)
                    };
                    break;
