...
public static class AdaptiveImageRenderer
{
    public static Control Render(AdaptiveImage image, AdaptiveRenderContext context)
    {
        ...
        var finalUri = context.Config.ResolveFinalAbsoluteUri(image.Url);
        if (finalUri == null) return uiImage;

        uiImage.SetImageSource(finalUri, context);

        uiImage.SetHorizontalAlignment(image.HorizontalAlignment);

        var style = $"Adaptive.{image.Type}";
        if (image.Style == AdaptiveImageStyle.Person)
        {
            ...
            {
                GradientOrigin = new(0.5, 0.5, RelativeUnit.Relative),
                Center = new(0.5, 0.5, RelativeUnit.Relative),
                Radius = 0.5,
                //RadiusX = 0.5,
                //RadiusY = 0.5,
                GradientStops = new()
            };
            mask.GradientStops.Add(new(Color.Parse("#ffffffff"), 1.0));
            mask.GradientStops.Add(new(Color.Parse("#00ffffff"), 1.0));
        ...
        if (image.PixelHeight > 0) uiImage.Height = image.PixelHeight;

        if (image.PixelWidth > 0) uiImage.Width = image.PixelWidth;
        ...
        {
            var color = Color.Parse(image.BackgroundColor);
            if (color.A != 0)
                uiBorder = new Border
                {
