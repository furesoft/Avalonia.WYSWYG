...
public static class AdaptiveDateInputRenderer
{
    public static Control Render(AdaptiveDateInput input, AdaptiveRenderContext context)
    {
        ...
        if (DateTimeOffset.TryParse(input.Value, out var date)) datePicker.SelectedDate = date;
        // textBox.SetPlaceholder(input.Placeholder);
        // textBox.Style = context.GetStyle($"Adaptive.Input.Text.Date");
        datePicker.SetContext(input);

        DateTimeOffset maxDate, minDate;
        if (DateTimeOffset.TryParse(input.Min, out minDate)) datePicker.MinYear = minDate;

        if (DateTimeOffset.TryParse(input.Max, out maxDate)) datePicker.MaxYear = maxDate;


        if ((DateTimeOffset.TryParse(input.Max, out maxDate) || DateTimeOffset.TryParse(input.Min, out minDate) ||
             input.IsRequired)
...
{
    public AdaptiveDateInputValue(AdaptiveDateInput inputElement, Control renderedElement) : base(inputElement,
        renderedElement)
    {
    }
    ...
    {
        var datePicker = RenderedInputElement as DatePicker;
    ...
    public override bool Validate()
    {
        ...
        var dateInput = InputElement as AdaptiveDateInput;
        ...
        else
        {
            ...
            if (!(!dateInput.IsRequired && string.IsNullOrEmpty(currentValue))) isValid = false;
        }
