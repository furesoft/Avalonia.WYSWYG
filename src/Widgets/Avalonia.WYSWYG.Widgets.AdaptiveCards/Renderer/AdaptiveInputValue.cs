...
public abstract class AdaptiveInputValue
{
    ...
    public virtual void ChangeVisualCueVisibility(bool inputIsValid)
    {
        ...
        {
            var tagContent = ErrorMessage.Tag as TagContent;
            RendererUtil.SetVisibility(ErrorMessage, !inputIsValid, tagContent);

            var helpText = "";
            if (!inputIsValid) helpText = ErrorMessage.Text;
...
/// <summary>
///     Abstract class that implements the Validate behaviour for isRequired in almost all inputValues (except for
///     Input.Toggle)
/// </summary>
public abstract class AdaptiveInputValueNonEmptyValidation : AdaptiveInputValue
{
    public AdaptiveInputValueNonEmptyValidation(AdaptiveInput inputElement, Control renderedElement) : base(
        inputElement, renderedElement)
    {
    }

    public AdaptiveInputValueNonEmptyValidation(AdaptiveInput input, Control renderedInput,
        Control visualElementForAccessibility) :
    ...
    public override bool Validate()
    {
        ...
        if (InputElement.IsRequired) isValid = !string.IsNullOrEmpty(GetValue());
...
{
    public AdaptiveTextBoxInputValue(AdaptiveInput inputElement, Control renderedElement) : base(inputElement,
        renderedElement)
    {
    }
    ...
    public override void ChangeVisualCueVisibility(bool isInputValid)
    {
        ...
        if (isInputValid)
            VisualErrorCue.BorderThickness = new(0);
        else
            VisualErrorCue.BorderThickness = new(2);
    }
}